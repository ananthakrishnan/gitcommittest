		...
		$.get(youtubeAPI,{'q':settings.videoID},function(response){
			...
			if(settings.progressBar){
				...
				elements.progress.click(function(e){
					...
					seekedTime									= Math.round(data.duration*ratio);
					
					console.log(startingTime+" n "+currentTime+" n "+seekedTime)
						
					if(seekedTime <= startingTime || startingTime === 0)
					{
						currentTime							= elements.player.getCurrentTime();
						
						if(elements.player.getCurrentTime() > startingTime)
						{
							startingTime						= elements.player.getCurrentTime();
						}
					...
					else if(seekedTime > startingTime)
					{
						if(currentTime === 0)
						{
							currentTime							= elements.player.getCurrentTime();
						...
						if(elements.player.getCurrentTime() > seekedTime)
						{
							startingTime						= elements.player.getCurrentTime();	
							currentTime							= 0;
						}
					...
					console.log(startingTime+" "+currentTime+" "+seekedTime)
					...
					if(!initialized)
					{
						...
						elements.control.click(function(){
							if(!elements.container.hasClass('playing')){
								...
								elements.player.seekTo(startingTime);
...
//					document.getElementById('ytplayer').addEventListener('onStateChange','eventListener_'+playerID);
//				}
//			}
