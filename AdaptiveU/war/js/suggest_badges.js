...
$(document).ready(function() 
					...
					if(indexUserBadgeDetails.indexOf(indexUserDetails) != -1)
						{
						 ...
						 if(String(userBadgelogMap[indexUserBadgeDetails].trophyId).indexOf(",") != -1)
							 ...
							 buildUserLi  += '<li class="admin">'+
											 ...
											 '</h4>'+
											 '<code><img height="42px" width="42px" src="'+userProfileMap[indexUserDetails].profilePicture+'"></code>'+
											 '</div>'+
											 ...
											 '<ul id="userBadges" class="userBadgeTrophyClass" style="max-height:125px;overflow:auto;">';
													for(indexUserStatusDetails in userStatusDetails)
													 	{
													 		if(String(userStatusDetails[indexUserStatusDetails].userId).indexOf(indexUserDetails) != -1 && String(badgesArray).indexOf(String(userStatusDetails[indexUserStatusDetails].stuffid)) != -1 && userStatusDetails[indexUserStatusDetails].status.indexOf('approved') != -1)
													 			  {
													 			  	 for(indexBadgeDetails in badgesListMap)
																	 {
													 			  		 if(indexBadgeDetails.indexOf(String(userStatusDetails[indexUserStatusDetails].stuffid)) != -1)
																	 			  	buildUserLi  += '<li id="'+indexUserDetails+'_'+indexUserStatusDetails+'" style="position: relative; " class="ui-draggable"><img src="'+badgesListMap[indexBadgeDetails].badgeLogoPath+'" id="'+indexBadgeDetails+'">'+
											 ...
											 '<ul id="userTrophies" class="userBadgeTrophyClass" style="max-height:125px;overflow:auto;">';
											 for(indexUserStatusDetails in userStatusDetails)
											 	{
											 		if(String(userStatusDetails[indexUserStatusDetails].userId).indexOf(indexUserDetails) != -1 && String(trophiesArray).indexOf(String(userStatusDetails[indexUserStatusDetails].stuffid)) != -1 && userStatusDetails[indexUserStatusDetails].status.indexOf('approved') != -1)
											 			  {
															 for(indexTrophyBadgeDetails in badgesListMap)
															 {
															 	if(badgesArray.indexOf(indexTrophyBadgeDetails) != -1 && String(badgesListMap[indexTrophyBadgeDetails].badgeType).indexOf('trophy') != -1)
															 		{
																 		for(indexUserStatusDetails in userStatusDetails)
															 			  {
																 			if(indexBadgeDetails.indexOf(String(userStatusDetails[indexUserStatusDetails].stuffid)) != -1)
																		 			buildUserLi  += '<li id="'+indexUserDetails+'_'+indexUserStatusDetails+'" style="position: relative; " class="ui-draggable"><img src="'+badgesListMap[indexTrophyBadgeDetails].badgeLogoPath+'" id="'+indexTrophyBadgeDetails+'">'+
   ...
   $( "#userTrophies > li").draggable({
   	...
   	for(indexBadgeDetails in badgesListMap)
   		{
   		if(String(badgesListMap[indexBadgeDetails].badgeType).indexOf("badge") != -1 && String(badgesListMap[indexBadgeDetails].badgeType).indexOf("deletedbadge") == -1)
   			{
		    ...
		    drop : function(event, ui)	
		     {
		    	...
		    	console.log("particularUser ::"+particularUser+" userStatusDetailsKey ::"+userStatusDetailsKey);
		    	for(index in badgesListMap)
				 	...
				 	$.getJSON("/deleteBadgeLogJDO?userStatusDetailsKey="+userStatusDetailsKey+"&badgeId="+droppedElementID+"&userId="+particularUser+"&badgeVal="+badgeVal+"&badgeType="+badgeType,function(result)
				 			{ 
				 		//Changes made now
   	 ...
   	 $( ".trophy_list").droppable({
		    ...
		    drop : function(event, ui)
		     {
		    	...
		    	var droppedElementID =ui.draggable.find('img').attr("id");
			     ...
			     $( ".badges_list").css("border" , "none");
			    	  $.getJSON("/deleteBadgeLogJDO?userStatusDetailsKey="+userStatusDetailsKey+"&badgeId="+droppedElementID+"&userId="+userKey+"&badgeVal="+badgeVal+"&badgeType="+badgeType,function(result)
			    			 { 
			    		  //Changes made now
			    		  	if(badgeType.indexOf("badge") != -1)
			    		  		{
		 ...
		 $(".badges_count").droppable({
			 ...
			 drop : function(event, ui)
			 {
						 ...
						 $.ajax({type: 'GET', url: "/updateUserStatusDetailsFromAdmin?badgeId="+droppedElementID+"&userKey="+particularUser+"&badgeVal="+badgeVal+"&badgeType="+badgeType+"&isSuggestedBadge=true", success: function(result)
							{
									 ...
									 console.log("spanTpInner ::"+spanTpInner);
						...
						//These functions are called to invoke the draggable feature for the created trophies
						 ...
						 /* $.ajax({type: 'POST', url: '/sendMailToUser' ,data:"badgeId="+droppedElementID+"&userId="+particularUser, success: function(data)
								{
									 $("#voice-box").fadeIn();
        				...
        				if(indexuserDetails.indexOf(String(userStatusDetails[index].userId)) != -1)
        				{
        					...
        					{	
        						if(indexBadgeDetails.indexOf(idOfTheElement) != -1)
        						{
    	        					...
    	        					{
    	        						buildUsersWorkingOn += '<p class="imageNameWrapper"><img src ="'+userProfileMap[indexuserDetails].profilePicture.replace("/photo.jpg","/s70-c/photo.jpg")+'" style="padding-bottom:5px;"/><br>'+userProfileMap[indexuserDetails].firstName+'  '+userProfileMap[indexuserDetails].lastName+'</p>';	
    	        					}
    	        						
    	        					else
    	        					{
    	        						buildUsersWorkingOn += '<p class="imageNameWrapper"><img style="max-height:80px;max-width:70px;" src ="/images/badge-robot.png"/><br>'+userProfileMap[indexuserDetails].firstName+'  '+userProfileMap[indexuserDetails].lastName+'</p>';
    	        					}
